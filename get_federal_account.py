...
if __name__ == '__main__':
    ...
    for agency in TOP_TIER:
        url = 'https://api.usaspending.gov/api/v2/agency/{}/federal_account/'.format(agency)
        result = get(url=url)
        result_json = result.json()
        LOGGER.info('%s %d', agency, len(result_json))
        data[agency] = result_json

    df = DataFrame(data=data).T.drop(columns=['messages'])
    output_file = OUTPUT_FOLDER + 'federal_accounts.json'
